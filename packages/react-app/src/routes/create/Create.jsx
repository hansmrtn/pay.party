...
const Create = ({ address, mainnetProvider, userSigner, tx, readContracts, writeContracts, targetNetwork }) => {
  ...
  const [toAddressVoters, setToAddressVoters] = useState(null);
  const [toAddressCandidates, setToAddressCandidates] = useState(null);
  ...
  //     const c = toAddressCandidates
  //     setCandidateAdrs(c.address);
  ...
  useEffect(() => {
    console.log(toAddressVoters)
    console.log(toAddressCandidates)
  }, [toAddressVoters, toAddressCandidates])
  ...
  const createForm = (
    <Box borderWidth={"1px"} shadow="xl" rounded="md" p="10" w="4xl" minW="sm">
      ...
      <FormControl id="create">
        ...
        /> */}       
        <Box bg='whiteAlpha.800' p='6' borderRadius={24}>
        <MultiAddressInput
          // autoFocus
          ensProvider={mainnetProvider}
          placeholder="Enter address"
          value={toAddressVoters}
          onChange={setToAddressVoters}
        />
        </Box>
        ...
        <MultiAddressInput
          ...
          ensProvider={mainnetProvider}
          placeholder="Enter address"
          value={toAddressCandidates}
          onChange={setToAddressCandidates}
        />
        <Center pt={10}>
      ...
      <form onSubmit={onSubmit}>
        <FormControl id="Review">
          ...
          <Box p="4" borderWidth="1px" w="min-content">
            {!isLoading ? (
              toAddressVoters?.map(d => {
                return (
                  <Text p="0.5" key={d.input}>
                    {d.input}
                  </Text>
                );
